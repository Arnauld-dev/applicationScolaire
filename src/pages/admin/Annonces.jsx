...
const Annonces = () => {
  ...
  const columns = [
    { key: 'titre', label: 'Titre', style: { width: '25%' } },
    { key: 'contenu', label: 'Contenu', style: { width: '35%' } },
    { key: 'type', label: 'Type', style: { width: '15%' }, render: (value) => <span className="badge bg-primary">{value}</span> },
    { key: 'classe', label: 'Classe', style: { width: '15%' } },
    { key: 'date', label: 'Date', style: { width: '10%' }, render: (value) => formatDate(value) },
    {
      key: 'actions',
      label: 'Actions',
      style: { width: '10%' },
  ...
  return (
    <div>
      ...
      <Card title="Annonces officielles">
        <Table
          columns={columns}
          data={annonces}
          striped
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="secondary" onClick={() => { setShowModal(false); setEditingAnnonce(null) }}>
              Annuler
            </Button>
      ...
      >
        <Form id="annonce-form" onSubmit={handleSubmit}>
          <Input label="Titre" value={formData.titre} onChange={(e) => setFormData({ ...formData, titre: e.target.value })} required />
          <Input label="Contenu" type="textarea" value={formData.contenu} onChange={(e) => setFormData({ ...formData, contenu: e.target.value })} required rows={5} />
          <Select label="Type" value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value })} options={['Officielle', 'Information', 'Urgente']} required />
          <Input label="Classe" value={formData.classe} onChange={(e) => setFormData({ ...formData, classe: e.target.value })} placeholder="Toutes classes" />
        </Form>
      </Modal>
    </div>
  )
